...
export default function SalesOrderFormPage({ mode, id }: Props) {
  ...
  const doConfirm = async () => {
    ...
    try {
      ...
      toast.error(extractApiError(err, "Failed to confirm sales order"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const doBackToDraft = async () => {
    ...
    setBusy(true);
    try {
      const updated = await draftSalesOrder(id);
      setStatus(updated.status);
      toast.success("Sales order moved back to draft");
    } catch (err) {
  ...
  const doCancel = async () => {
    ...
    try {
      ...
      toast.error(extractApiError(err, "Failed to cancel sales order"));
    } finally {
      setBusy(false);
    }
  };
